...
const Drop = ({ label, items, base }: { label: string; items: MenuItems; base: string }) => (
  <details className="group relative">
    <summary className="flex cursor-pointer list-none items-center gap-1 rounded px-2 py-2 marker:hidden [&::-webkit-details-marker]:hidden">
      {label}<ChevronDown className="h-3.5 w-3.5 transition group-open:rotate-180" />
    </summary>
    <div className="absolute left-0 top-full z-50 mt-1 max-h-[70vh] w-72 overflow-auto rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
      <Link href={base} className="block rounded-lg px-3 py-2 text-sm font-bold hover:bg-slate-50">View All {label}</Link>
...
export const HeaderNav: React.FC = () => {
  ...
  return <>
    <nav className="hidden items-center gap-1 text-sm font-semibold md:flex">
      ...
      <Drop label="Services" items={services} base="/services"/>
      <Drop label="Industries We Serve" items={industries} base="/industries"/>
      <Drop label="Portfolio" items={portfolio} base="/portfolio"/>
      <Drop label="Process" items={process} base="/process"/>
      <Drop label="Blog" items={blog} base="/blog"/>
      ...
      <Drop label="Legal" items={legal} base="/privacy-policy"/>
      <Link className="rounded px-2 py-2 hover:bg-slate-50" href="/search" aria-label="Search"><Search className="h-4 w-4" /></Link>
    </nav>
    <div className="md:hidden">
      <button type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-label={open ? 'Close menu' : 'Open menu'} className="grid h-10 w-10 place-items-center rounded-lg border border-slate-200 bg-white text-[#10234d] shadow-sm">
        <Menu className="h-5 w-5" />
      </button>
      {open && <div className="absolute right-4 top-full z-50 mt-2 max-h-[80vh] w-[min(92vw,360px)] overflow-auto rounded-xl border border-slate-200 bg-white p-3 shadow-2xl">
        <div className="grid gap-1 text-sm font-semibold">
          ...
          {([['Services',services,'/services'],['Industries We Serve',industries,'/industries'],['Portfolio',portfolio,'/portfolio'],['Process',process,'/process'],['Blog',blog,'/blog'],['Legal',legal,''] ] as const).map(([label,items,base]) => (
            <details key={label} className="rounded-lg border border-slate-100">
              <summary className="cursor-pointer px-3 py-3 font-bold">{label}</summary>
              <div className="grid border-t border-slate-100 p-2">
                {items.map(([title,slug]) => <Link key={slug} onClick={() => setOpen(false)} className="rounded px-3 py-2 text-sm hover:bg-slate-50" href={label === 'Legal' ? `/${slug}` : `${base}/${slug}`}>{title}</Link>)}
              </div>
            </details>
          ))}
