...
export default async function KaanchwalaHome() {
  ...
  return <main>
    ...
    <section className="container py-16"><h2 className="mb-6 text-sm font-black uppercase tracking-wide text-[#0644a4]">Types of Office Glass Partitions</h2><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{services.map((service, i) => <a href={`/services/${service.slug}`} key={service.slug || service.title} className="group block cursor-pointer overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md"><img draggable={false} src={image(['photo-1497366811353-6870744d04b2','photo-1497366216548-37526070297c','photo-1497366754035-f200968a6e72'][i % 3])} alt={service.title} className="pointer-events-none h-36 w-full select-none object-cover"/><div className="p-4"><h3 className="font-extrabold group-hover:text-[#0644a4]">{service.title}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{service.summary}</p><span className="mt-3 block text-xs font-bold text-[#0644a4]">VIEW SERVICE →</span></div></a>)}</div><Link href="/services" className="mt-6 inline-block rounded-md border border-[#0750b8] px-6 py-3 text-sm font-bold text-[#0750b8]">VIEW ALL SERVICES →</Link></section>
