...
const fallbackServices = [
  ['Frameless Glass Partitions', 'Sleek, minimal design with maximum transparency', 'glass-partitions'],
  ['Aluminium Framed Partitions', 'Durable frames with a modern clean finish', 'aluminium-framed-partitions'],
  ['Half Glass Partitions', 'Privacy-focused partition systems for offices and homes', 'glass-partitions'],
  ['Sliding Glass Partitions', 'Flexible sliding solutions for changing spaces', 'glass-partitions'],
  ['Double Glazed Partitions', 'Extra acoustic separation for private discussions', 'glass-partitions'],
  ['Curved Glass Partitions', 'Custom curved glass for distinctive interiors', 'glass-partitions'],
] as const

const serviceSlugAliases: Record<string, string> = {
  'frameless-glass-partitions': 'glass-partitions',
  'aluminium-framed-partitions': 'glass-partitions',
  'half-glass-partitions': 'glass-partitions',
  'sliding-glass-partitions': 'glass-partitions',
  'double-glazed-partitions': 'glass-partitions',
  'curved-glass-partitions': 'glass-partitions',
}

const canonicalServiceSlug = (slug: string, title: string) => serviceSlugAliases[slug] || (title.toLowerCase().includes('partition') ? 'glass-partitions' : slug)
...
export default async function KaanchwalaHome() {
  ...
  const services = serviceData.docs.length
    ? serviceData.docs.map((item) => ({ title: item.title, summary: item.summary, slug: canonicalServiceSlug(item.slug, item.title) }))
  ...
  return <main>
    ...
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="container flex min-h-14 items-center justify-between gap-3 py-1">
        <Link href="/" aria-label={siteName} className="flex h-16 w-16 shrink-0 items-center justify-center sm:h-[76px] sm:w-[76px]">
          <Logo loading="eager" priority="high" className="h-16 w-16 sm:h-[72px] sm:w-[72px]" />
        </Link>
